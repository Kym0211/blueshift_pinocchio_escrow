use pinocchio::{account_info::AccountInfo, instruction::Seed, msg, program_error::ProgramError, pubkey::find_program_address, ProgramResult};
use pinocchio_token::instructions::Transfer;

use crate::{state::Escrow, AccountCheck, AssociatedTokenAccount, AssociatedTokenAccountCheck, AssociatedTokenAccountInit, MintInterface, ProgramAccount, ProgramAccountInit, SignerAccount};
...
impl<'a> TryFrom<&'a [AccountInfo]> for MakeAccounts<'a> {
    ...
    fn try_from(accounts: &'a [AccountInfo]) -> Result<Self, Self::Error> {
        // Log the number of accounts and their keys for debugging
        msg!(&format!("Number of accounts: {}", accounts.len()));
        for (i, acc) in accounts.iter().enumerate() {
            msg!(&format!("Account {} : {:?}", i, acc.key().as_ref()));
        }

        let [maker, escrow, mint_a, mint_b, maker_ata_a, vault, system_program, token_program, extra] = accounts else {
